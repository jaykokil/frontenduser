...
export default function App() {
  ...
  async function login(username, password) {
    try { const result = await apiRequest("/auth/login", { method: "POST", body: JSON.stringify({ username, password }) }); localStorage.setItem("token", result.token); localStorage.setItem("inventory_user", JSON.stringify(result.user)); setSession({ type: "user", userId: result.user.id }); return { ok: true }; }
    catch (error) { return { ok: false, message: error.message }; }
  }
  function logout() { localStorage.removeItem("token"); localStorage.removeItem("inventory_user"); setSession(null); }
  useEffect(() => { const token = localStorage.getItem("token"); const user = getStoredUser(); if (token && user) setSession({ type: "user", userId: user.id }); }, []);
